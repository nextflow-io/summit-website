...
const Pixels = forwardRef(
  (
    ...
    const { toggleTile } = usePacmanGame(
      canvasRef,
      cellSize,
      gridCols,
      gridRows,
          ...
          Array(newRows)
            ...
            .map((_, j) => {
              if (i < oldCols && j < oldRows)
                return fullAgeGridRef.current?.[i]?.[j] || 0;
    ...
    useEffect(() => {
      ...
      if (pacmanMode && !prevPacmanModeRef.current) {
        if (gridRef.current) {
          ...
          savedAgeGridRef.current =
            ageGridRef.current?.map((col) => [...col]) || null;
        }
      }
      ...
      // Create or resize grid as needed
      if (!gridRef.current) {
        gridRef.current = createRandomGrid(cols, rows, density);
      } else if (
      ...
      if (!fc || cols > fc.length || rows > (fc[0]?.length || 0)) {
        ...
        fullGridRef.current = Array(fCols)
          .fill(null)
          .map((_, i) =>
            Array(fRows)
              .fill(null)
        ...
        fullAgeGridRef.current = Array(fCols)
          .fill(null)
          .map((_, i) =>
            Array(fRows)
              .fill(null)
