...
const Tabs = ({ children, location, anchor }) => {
  return (
    <div className="bg-black rounded-sm overflow-hidden inline-flex">
      <TabsContext.Provider value={{ location, anchor }}>{children}</TabsContext.Provider>
    </div>
  );
};
  ...
  return (
    <Link
      ...
      className={classnames(
        ...
        {
          'bg-black text-green-600 hover:bg-gray-800': !location.pathname?.includes(to) || inactive,
          'bg-green-600 text-white': location.pathname?.includes(to) && !inactive,
        }
      )}
    >
      {children}
